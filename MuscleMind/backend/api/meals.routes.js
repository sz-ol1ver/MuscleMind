...
// sajat recept szerkesztese
router.put('/edit/:id', upload.none(), loginMw.requireAuthApi, foodsMw.validateNewFood, async (request, response) => {
    try {
        const foodId = request.params.id
        const userId = request.session.user.id
        const food = request.body

        const affectedRows = await db.updateUserFood(userId, foodId, food)
        
        if (affectedRows === 0) {
            return response.status(404).json({
                message: 'Étel nem található, vagy nincs jogosultságod szerkeszteni!'
            })
        }

        // allergenek frissitese (torles es ujra beszuras)
        await db.deleteFoodAllergens(foodId)
        for (let allergen of food.allergens) {
            await db.insertFoodAllergen(foodId, allergen)
        }
        ...
        await db.log_id(
            userId,
            'meals - food edit',
            'edited user food id: ' + foodId + ' | name: ' + food.name,
            ip
        )
        ...
        const ip = requestIp.getClientIp(request)
        try {
            await db.log_error('Server error - meals - edit', error.message, ip)
        } catch (err) {
            console.error('Logging failed:', err)
        }
        return response.status(500).json({
